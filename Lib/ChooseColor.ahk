...
/*!
    ...
    Parameters:
        ...
        Palette - An array of up to 16 RGB color values. These become the 
                  initial custom colors in the dialog. The index should be linear at begin at 1 ie 1-16
                  If user selects OK this custom colours are saved back into this array
    ...
    Remarks:
        ...
        The Window's ChooseColour API uses BGR even though it talks about RGB
...
/*
    Custom Colour 
    http://msdn.microsoft.com/en-us/library/windows/desktop/dd183449(v=vs.85).aspx
...
ChooseColor(pRGB := 0, hOwner := 0, DlgX := 0, DlgY := 0, Palette := "")
{
    ...
    if !hasRun ; So any unfilled custom colours in the palette will be white instead of black
    {
        hasRun := True
        loop 16
...
;___Save the changes made to the custom colors
    if !ErrorLevel
    {
        Loop 16 ; If Palette isn't an array doesn't matter.
